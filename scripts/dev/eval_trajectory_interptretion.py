...
if __name__ == "__main__":
    ...
    for i in range(4):
        vid_fname = f"sim_{i}.mp4"
        ball_pos_true = load_sim_ball_pos(i, ROOT_DIR_PATH, N_FRAMES_TO_AVERAGE)

        rng = np.random.default_rng()

        # Add noise to true ball positions to simulate measurements
        noisy_gt = np.copy(ball_pos_true) + rng.normal(
            0, 0.05, size=(ball_pos_true.shape[0], 3)
        )
        ...
        for j in range(phi.shape[0]):  # Set main diagonal to ones
            phi[j, j] = 1
        mu_m = np.zeros((measurements_dim, 1))
        measurement_noise = rng.normal(
            0, 0.07, size=(measurements_dim, measurements_dim)
        )
        sigma_m = (np.identity(measurements_dim) * 8) + measurement_noise
        ...
        ti = TrajectoryInterpreter(
            kalman_filter=kf, n_dim_samples=[9, 9, 9], n_std_devs_to_sample=2
        )
        classification = ti.classify_trajectory(
            0.2, visualise=False, show_sample_points=True
        )

        print(f"Sim ID {i}'s trajectory has been interpreted as - {classification}")
